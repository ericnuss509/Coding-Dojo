...
class User:
    ...
    def user_id(cls,data):
        ...
        result = connectToMySQL('game_db').query_db(query,info)
        print(result)
        return result

    @classmethod
    def get_one_by_id(cls,id):
        query = 'SELECT * FROM users WHERE id = %(users_id)s;'  # SELECT queries will return the data from the database as a LIST OF DICTIONARIES
        data = {
            "users_id": id
        }
